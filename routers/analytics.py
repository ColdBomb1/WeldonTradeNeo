...
from services import performance_service, risk_manager, claude_service
...
def run_claude_performance_review() -> JSONResponse:
    """Ask Claude to analyze overall trading performance."""
    cfg = load_config()
    if not cfg.claude.enabled:
        return JSONResponse({"error": "Claude API not enabled"}, status_code=400)
    ...
    analysis = claude_service.analyze_strategy_performance(scorecard, accuracy)
